...
{   
    ...
    SYS_JIAOZHUNYINZI xz;
     char temp[5] = {0};
    xz.DI_A = FloatToSmall(SysRunState.stParam.s_SysParam.Canshu1);
    xz.DI_B = FloatToSmall(SysRunState.stParam.s_SysParam.Canshu2);
    ...
    //memcpy(&cs.PingHuaShiJian,&SysRunState.stParam.s_SysParam.PingHuaShiJian,4);
     //memcpy(temp,SysRunState.stParam.s_SysParam.yuzhi1, 4);
    xz.DET_THR_1 = atoi(temp);
    memcpy(&xz.DET_THR_1,&SysRunState.stParam.s_SysParam.yuzhi1,4);
    //memcpy(&xz.DET_THR_2 ,&SysRunState.stParam.s_SysParam.yuzhi2,4);

    memcpy(temp,SysRunState.stParam.s_SysParam.yuzhi2, 4);
    xz.DET_THR_2 = atoi(temp);
    //memcpy(&xz.DET_TIME,&SysRunState.stParam.s_SysParam.PingHuaShiJian,4);

    memcpy(temp,SysRunState.stParam.s_SysParam.PingHuaShiJian, 4);
    xz.DET_TIME = atoi(temp);
        

    
	//SendData('R',(uint8_t*)&xz,sizeof(SYS_PRAM));
    SendData('R',(uint8_t*)&xz,sizeof(SYS_JIAOZHUNYINZI));
}
...
void ACK_CMD_W(unsigned char *cdata)
{
    ...
    SysRunState.stParam.s_SysParam.Canshu1 = FloatToSmall(xz.DI_A);
    SysRunState.stParam.s_SysParam.Canshu2 = FloatToSmall(xz.DI_B);

    sprintf(temp, "%04u", (unsigned int)xz.DET_THR_1);
    memcpy(SysRunState.stParam.s_SysParam.yuzhi1, temp, 4);

        sprintf(temp, "%04u", (unsigned int)xz.DET_THR_2);
    memcpy(SysRunState.stParam.s_SysParam.yuzhi2, temp, 4);

        sprintf(temp, "%04u", (unsigned int)xz.DET_TIME);
    memcpy(SysRunState.stParam.s_SysParam.PingHuaShiJian, temp, 4);
